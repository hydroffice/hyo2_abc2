...
class NOAASupport:
    ...
    caris_root = "C:\\CARIS"
    support_version = "2026.2"

    @classmethod
    def v_version(cls):
        return cls.support_version.replace(".", "v")

    def __init__(self, app_info: PkgInfo, progress=CliProgress()):
        self._ai = app_info
        self.progress = progress
        self.local_zip_path = os.path.abspath(os.path.join(PkgHelper(pkg_info=self._ai).pkg_folder(),
    ...
    def unzip_internal_zip(self):
        ...
        try:
            ...
            unzip_path = PkgHelper(pkg_info=self._ai).pkg_folder()
    ...
    def local_noaa_support_folder(self):
        ret = str()
        for folder in sorted(os.listdir(PkgHelper(pkg_info=self._ai).pkg_folder())):
            # logger.debug("folder: %s" % folder)
            if ("Caris_Support_Files" in folder) and (len(folder) > 24):
                ret = os.path.join(PkgHelper(pkg_info=self._ai).pkg_folder(), folder)
    ...
    def delete_local_noaa_support_files(self):
        ...
        for folder in os.listdir(PkgHelper(pkg_info=self._ai).pkg_folder()):
            ...
            candidate_path = os.path.join(PkgHelper(pkg_info=self._ai).pkg_folder(), folder)
    ...
    def open_local_noaa_support_folder(self):
        if self.local_noaa_support_folder_present():
            PkgHelper.explore_folder(self.local_noaa_support_folder())
            return

        PkgHelper(pkg_info=self._ai).explore_pkg_folder()
    ...
    def open_system_noaa_support_folder(cls):
        if cls.system_noaa_support_folder_present():
            PkgHelper.explore_folder(cls.system_noaa_support_folder())
            return
