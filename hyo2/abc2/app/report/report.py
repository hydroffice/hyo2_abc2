...
class Report:
    ...
    def generate_pdf(self, path: str, title: str = "Document", use_colors: bool = False, small: bool = False) -> bool:
        ...
        try:
            ...
            logger.warning("PySide6 is not properly installed, thus you cannot create a pdf file")
            self.display()
            return False
        ...
        if '.pdf' not in path.lower():
            ...
            return False
        path = PkgHelper.truncate_too_long(path)
        ...
        printer.setPageSize(QtGui.QPageSize.A4)
        ...
        def print_page_template():
            ...
            # logo
            hyo_logo_path = os.path.join(app_info.app_media_path, 'poweredby.png')
        ...
        for content_item in self.records:
            ...
            if last_item.isdigit():
                ...
                # noinspection PyTypeChecker
                painter.drawText(row_area, lc_flags, "%s.%s. %s"
                                 % (self.alphabet[section_nr], subsection_nr, content_item.rsplit(' ', 1)[0]))
                painter.setFont(normal_font)
                subsection_nr += 1
